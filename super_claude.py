#!/usr/bin/env python3
"""
super-claude — Claude Code Knowledge Architecture Bootstrap (v4)
...
def read_existing_content(root: Path) -> dict:
    ...
    if claude:
        ...
        is_self_generated = "generated_by:" in claude[:500] and "super-claude" in claude[:500]
    ...
    if standards:
        is_self_standards = "super-claude" in standards[:300]
...
def generate_claude_md(info: dict) -> str:
    ...
    L = [
        ...
        f"tier: T{tier_num}",
        f"generated_by: super-claude v1.2",
    ...
    L.extend(["---", "", "## Document Information", "",
              ...
              f"**Generated by:** super-claude v1.2"])
...
def generate_ralph_prompt_md(info: dict) -> str:
    ...
    L = [
        ...
        "root-level knowledge architecture files. Read them in this order:", "",
        "1. **[../CLAUDE.md](../CLAUDE.md)** — Primary project context (architecture, stack, conventions)",
        "2. **[../STANDARDS.md](../STANDARDS.md)** — QA rules, tier system, naming conventions",
        "3. **[../ERRORS_AND_LESSONS.md](../ERRORS_AND_LESSONS.md)** — Mistakes catalog (read BEFORE starting work)",
        "",
        ...
        "```", "",
        "### Error Handling",
        "- If you encounter a recurring error, add it to `../ERRORS_AND_LESSONS.md`",
        ...
        "1. Update `../QUICKSTART.md` if new commands were added (only if it has the AUTO-MAINTAINED marker)",
        "2. Update `../CLAUDE.md` if architecture changed",
        "3. Add errors/lessons to `../ERRORS_AND_LESSONS.md`",
    ]
    return "\n".join(L) + "\n"
...
def generate_ralphrc(info: dict) -> str:
    ...
    L = [
        ...
        f"# Generated by super-claude for {name}",
        "",
        "# ── Project identity ──",
        f'PROJECT_NAME="{name}"',
...
def setup_ralph(target: Path, info: dict, dry_run: bool = False, force: bool = False) -> list:
    ...
    actions.append(FileAction(".ralph/PROMPT.md", "overwrite" if prompt_path.exists() and not dry_run else "create", lines=content.count("\n")))

    # 2. .ralph/AGENT.md → ../QUICKSTART.md symlink
    agent_path = ralph_dir / "AGENT.md"
    if not dry_run:
        if agent_path.exists() or agent_path.is_symlink():
            agent_path.unlink()
        agent_path.symlink_to("../QUICKSTART.md")
    actions.append(FileAction(".ralph/AGENT.md", "create", reason="symlink → QUICKSTART.md"))

    # 3. .ralph/fix_plan.md — only create if doesn't exist (Ralph owns this file)
    fix_plan_path = ralph_dir / "fix_plan.md"
    if fix_plan_path.exists() and not force:
    ...
    actions.append(FileAction(".ralph/hooks/post-loop.sh", "create", lines=content.count("\n")))
    ...
    if ".ralph/logs/" not in existing:
        ...
        actions.append(FileAction(".gitignore", "update", reason="added Ralph entries"))
